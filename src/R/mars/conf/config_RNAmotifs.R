...
required_packages <- c("lattice", "latticeExtra", "bootstrap", "parallel")
optional_packages <- c("ggplot2", "reshape2")
...
# --- Logo (compatibility stub for old scripts) --------------------------------
...
getTables <- function(protein, place, tets, inExon = NULL, inIntron = NULL) {
    ...
    # Use dynamic row positions if inExon/inIntron provided, else ROWN constant
    if (!is.null(inExon) && !is.null(inIntron)) {
        region_spacing <- 2 * max(inExon, inIntron) + 10
        regions <- cumsum(c(region_spacing, rep(region_spacing, 3)))
        my_rown <- c((regions[1] - inExon):(regions[1] + inIntron),
    ...
    rown <- as.character(my_rown)
    tab  <- matrix(0, nrow = length(my_rown), ncol = length(sel_tets),
                   dimnames = list(rown, sel_tets))
    res  <- list(enh = tab, sil = tab, cont = tab)

    for (i in seq_along(filelist)) {
        fpath <- paste0(protein, place, filelist[i])
        df <- tryCatch(read.delim(fpath, fill = TRUE, header = FALSE),
    ...
    xyplot(s ~ plot | g, data = pf_real,
           ...
           panel = function(x, y, hrect, rcol, intensity, groups,
               ...
               panel.xblocks(
                   ...
                   c(rep("0", exon), rep(NA, intron+1), rep(NA, gap),
                     rep(NA, intron+1), rep("3", exon), rep(NA, gap),
                     rep("4", exon), rep(NA, intron+1), rep(NA, gap),
                     rep(NA, intron+1), rep("7", exon)),
                   col = "gray96", border = FALSE, lwd = 0.5)
               panel.xyplot(x, y, ...)
               panel.xblocks(
                   x = offset$start[1]:offset$end[4],
                   c(rep(NA, exon+intron+1), rep("0", gap), rep(NA, exon+intron+1),
                     rep("0", gap), rep(NA, exon+intron+1), rep("0", gap),
                     rep(NA, exon+intron+1)),
